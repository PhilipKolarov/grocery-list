function attachEvents() {
    ...
    let currentProducts = []; // array of objects (products)
    let productToEdit = {};
    ...
    function loadUpdateFormHandler() {
        ...
        //.find - will return the first element that answers certain requirements
    ...
    function updateProductHandler(event) {
        ...
        fetch(`${BASE_URL}${productToEdit._id}`, httpHeaders)
            .then(() => {
                ...
                otherDOMSelectors.form.reset(); // removes filled in form once submitted (resets it)
            })
            .catch((err) => {
                console.error(err);
            })
